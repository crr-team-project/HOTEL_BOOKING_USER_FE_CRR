...
export const FavoritesProvider = ({ children }) => {
 ...
 const isFavorite = (hotelId) => {
  const result = favorites.some((fav) => {
   const favHotelId = fav.hotelId?.id || fav.hotelId?._id || fav.hotelId;
   return favHotelId === hotelId;
  });
  return result;
 };

 const getFavoriteByHotelId = (hotelId) => {
  const result = favorites.find((fav) => {
   const favHotelId = fav.hotelId?.id || fav.hotelId?._id || fav.hotelId;
   return favHotelId === hotelId;
  });
  return result;
 ...
 const value = {
  ...
  toggleFavorite,
  favoriteHotelIds: favorites.map(
   (fav) => fav.hotelId?.id || fav.hotelId?._id || fav.hotelId
  ),
 };
